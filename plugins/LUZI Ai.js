...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to Luzi Mwol voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Raashii.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Luzi') && conf.LUZI_AI !== 'true') {        
        ...
        var finm = message.message.replace('Luzi', '').replace(' ', '')   
...
Raashii.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.LUZI_AI == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel Luzi özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Luzi yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Luzi yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'Luzi, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'Luzi, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Luzi Mwolfeatures. Turn your account into a ai chatbot!'
    already_on = 'Luzi Mwol artificial intelligence is already fully functional.'
    already_off = 'Luzi Mwol artificial intelligence is currently running semi-functional.'
    succ_on = 'ʟᴜᴢɪ ᴄʜᴀᴛ ʙᴏᴛ ᴏɴ! ✅'
    succ_off = 'Luzi Mwol Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Luzi-Mwol സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Luzi Mwol കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Luzi Mwol നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Luzi Mwol പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Luzi Mwol സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Raashii.addCommand({ pattern: 'luzi chat bot ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.luzi bot on / off' }, (async (message, match) => {
    var pinky_status = `${conf.LUZI_AI}`
    if (match[1] == 'on') {
